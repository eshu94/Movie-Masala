...
class IsoLocaleCodes {
     func find( key:String ) -> IsoLocaleInfo {
        let locale = IsoLocales.allLocales.filter(  { $0.alpha1 == key.lowercased() || $0.alpha2 == key.lowercased() || $0.alpha3 == key.lowercased() } )
     ...
     func searchByName( name:String ) -> IsoLocaleInfo {
        let country = IsoLocales.allLocales.filter( { $0.name.lowercased() == name.lowercased() } )
